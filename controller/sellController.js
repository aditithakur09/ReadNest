...
exports.uploadImage = (req, res) => {
    ...
    db.query(sql, [book_id, imagePath], (err) => {
        if (err) return res.status(500).json({ msg: "Error saving image" });
...
exports.adminApprove = (req, res) => {
    const {
        book_id, offered_price,
        buyer_name, buyer_mobile,
        buyer_address, admin_id
    } = req.body;

    if (!book_id || !offered_price)
        return res.status(400).json({ msg: "Required: book_id & offered_price" });
    ...
    db.query(
        sql,
        [offered_price, buyer_name || null, buyer_mobile || null,
        buyer_address || null, admin_id || null, book_id],
        (err) => {
...
// USER: Accept Approved Offer
exports.userAccept = (req, res) => {
    const { book_id, user_id } = req.body;

    const sql = `SELECT user_id, status FROM books_for_sell WHERE id=?`;

    db.query(sql, [book_id], (err, rows) => {
        if (err) return res.status(500).json({ msg: "Error checking book" });
        ...
        db.query(
            `UPDATE books_for_sell SET status='accepted', sold_date=NOW() WHERE id=?`,
            [book_id],
            (err2) => {
                if (err2) return res.status(500).json({ msg: "Error updating" });
...
// USER: Reject Approved Offer
exports.userReject = (req, res) => {
    const { book_id, user_id, rejection_reason } = req.body;

    const sql = `SELECT user_id, status FROM books_for_sell WHERE id=?`;

    db.query(sql, [book_id], (err, rows) => {
        if (err) return res.status(500).json({ msg: "Error checking" });
        if (!rows.length) return res.status(404).json({ msg: "Book not found" });
        if (rows[0].user_id != user_id)
            return res.status(403).json({ msg: "Not authorized" });
        if (rows[0].status !== "approved")
        ...
        db.query(
            `UPDATE books_for_sell SET status='rejected', rejection_reason=? WHERE id=?`,
            [rejection_reason || null, book_id],
            (err2) => {
                if (err2) return res.status(500).json({ msg: "Error updating" });
                res.json({ msg: "You rejected the offer" });
            }
        );
    });
};
